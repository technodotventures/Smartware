...
| 2026-09-15 | `tsc` clean · **541/541** vitest across 75 files · `verify:schemas` 31 OK · `verify:saas` SMOKE_OUTCOME=pass · `npm audit` 0 prod vulns · `status:check` current at commit time (re-stales with live-board churn) · legal-hold verification findings fixed (F1 release requires `operation_id`; F2 replay converges the released duty + atomic+fsync config writes; F3 v0.5.0 ops enum completed) · first full run had 1 infra flake (10 s `beforeEach` hook timeout in `mcp_smoke.test.ts` at host load ~20; file passes in isolation and the re-run is 541/541) | kanban task `t_7a64ded2`, branch `wip/neo/legal-hold-findings`, evidence `/opt/data/dev-workspaces/gate-t_7a64ded2.log` + `/opt/data/dev-workspaces/test-t_7a64ded2-rerun.log` |
| 2026-09-15 | `tsc` clean · **542/542** vitest across 75 files · `verify:schemas` 31 OK · `verify:saas` SMOKE_OUTCOME=pass · `npm audit` 0 prod vulns · writer-surface `OpType` pinned to the published v0.5.0 `op` enum (`OP_TYPES` runtime list; four dead members removed; no schema file changed) · MCP missing-field refusal wording corrected (transport `-32602` before the handler; core `invalid_parameter`) · `status:check` current at commit time (re-stales with live-board churn) | kanban task `t_0e3989eb`, branch `wip/neo/ops-enum-symmetry`, evidence `/opt/data/dev-workspaces/t_0e3989eb-evidence/` |
| 2026-09-15 | `tsc` clean · **542/542** vitest across 75 files · `verify:schemas` 31 OK · `verify:saas` SMOKE_OUTCOME=pass · `npm audit` 0 prod vulns · writer-surface `OpType` derivation guarded at compile time (F-1 of `t_2b8776f7`, closed by `t_9f0f314c`): widening it past `OP_TYPES` now fails `npm run build` (`TS2322` on the proof) instead of staying tsc-clean with the enum pin green — re-inline and list-side mutations measured; no schema file changed (`schemas/v0.5.0/SHA256SUMS` blob identical) · `status:check` green immediately before this commit (re-stales with live-board churn; the board event counter moves between consecutive projections) | kanban task `t_9f0f314c`, branch `wip/neo/optype-derivation-guard`, evidence `/opt/data/dev-workspaces/t_9f0f314c-evidence/` |

| 2026-09-15 | `tsc` clean · `verify:schemas` 31 OK · `verify:saas` SMOKE_OUTCOME=pass · `npm audit` 0 prod vulns · local full run on the shared box at load ~160–180 was 530/542 (`4 failed | 71 passed` files) — all 12 failures hook/`testTimeout`-class in 4 heavy conformance files, **no assertion failures**: `v050-rebuild-forget-provenance` **14/14** and `q_lifecycle_composition` **8/8** pass in isolation, `p0_isolation_conformance` **11/11** (`--testTimeout=180000`) and `mcp_smoke` **6/6** (`--hookTimeout=60000`) pass with load-appropriate budgets, and `mcp_smoke` fails identically on the pre-fix revision `3822f4b` (full suite on CI Node 22+24 is the authoritative run — PR #18) · writer-surface `OpType` derivation guard hardened (F-A/F-A'/F-B of `t_037255f6`, closed by `t_0e732b96`): dropping the list's ` as const`, annotating it `readonly string[]`, or erasing the derivation with `| any` each reached an enum-foreign writer op (no-cast `CommitDescriptor` site emitting `watch.subscribe`) with `npm run build` exit 0 pre-fix; post-fix all three and the card's original repro fail `npm run build` at the proof (`TS2322`), comment-only control stays exit 0 · no schema file changed (`schemas/` tree hash `a701e576...` identical at `265b00a`/`ff8f901`) · `status:check` green immediately before this commit (re-stales with live-board churn) | kanban task `t_0e732b96`, branch `wip/neo/optype-derivation-guard`, evidence `/opt/data/kanban/boards/smartware/workspaces/t_0e732b96/evidence/` |
