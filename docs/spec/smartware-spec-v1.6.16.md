...
**Re-picking the survivor (v0.5.0).** Beta's one explicit release of a mechanical demotion (a duplicate resolved by the fact-identity rule, §1e of the integration guide) is a user-only REVISE action — `repick_survivor: true` on the **demoted** duplicate. One atomic commit appends a version of the duplicate with its `superseded_by`/`superseded_at` cleared (plus the audit-only `reinstated_by: 'user'`, and `epistemic_owner: user` from the adjudication), and a version of the fact's current active copy demoted with the warrant `superseded_by_origin: 'user'`; exactly one copy of the fact stays recall-eligible, and the next write touching that fact does not re-demote the release. The act is the operation's only action in beta, and — like the mechanical demotion itself — it deliberately does **not** admit a `supersedes` edge, so it is outside the effective-current rule above and `invalidate_relations` neither touches nor undoes it. A target whose latest version carries no `superseded_by` is rejected (`not_demoted`, nothing written); if the previous survivor is already forgotten, the release happens alone (rescue mode). The normative detail lives in the v0.5.0 protocol contract (`docs/protocol/smartware-protocol-v0.5.0.md` → REVISE) and ADR-0003 → *Releasing a demotion*; the schemas are `schemas/v0.5.0/revise-request.schema.json` and `schemas/v0.5.0/claim.schema.json`.
