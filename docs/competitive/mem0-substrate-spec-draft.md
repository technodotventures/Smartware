...
**Status:** v0.13 NORMATIVE · 2026-08-29 · G0 verdict + compile-latency evidence baked in (§11); Coffee tenant config shape bound (§10b); client-scope flows + provenance-rendering contract bound (§10c/§10d), F1 export surface SHIPPED (G3.1); all §9 decisions closed; supersedes DRAFT (v0.3–v0.6)
...
- **Binding: a dispute / legal-hold trigger NEVER executes erasure.** It executes the hold lane: offboarding (tombstone + grant revoke, data retained) + F1 export snapshot held in the Pod. Erasure runs only when the hold releases — the Coffee flow requires an owner attestation ("no pending dispute / verified request / hold released"), recorded in the ops entry; the snapshot is the defense record. **Implemented (t_ad51d0e2):** `FORGET.SCOPE { reason: 'erasure', attestation }` — the string is recorded as `details.attestation` in the ops entry (erasure lane only; rejected on offboarding; empty is refused). **Amended by ADR-0009 (2026-09-15):** the marker now exists, so erasure is gated by hold state (`legal_hold_open`) and the release act (`hold.release { statement }`) is the canonical home for that statement; the optional erasure `attestation` field is kept for payload identity and is no longer the only refusal seam.
...
- **Superseded by the ADR-0009 marker (2026-09-15): the explicit legal-hold marker IS built — see the implementation bullet below.** History — decided (ADR-0008, 2026-09-14) that the explicit legal-hold marker was NOT built; v1 composition was the standing answer. Measured on a live core with an elapsed time-bound observation: (a) the hold lane (offboarding) tombstones the scope's observations itself, so a sweep over the held scope expires nothing — the skip is by construction, not a flag the sweep consults; (b) evidence written into the scope *after* the hold is not skipped and an elapsed sweep tombstones it — non-destructively (bytes appended, nothing removed) with the held scope still exportable in full, so the defense record survives; (c) erasure on a held scope is not refused by the substrate — it is an owner act whose statement (or its absence, `attestation: null`) is recorded. The substrate's guarantee was therefore **"never silent"**, not "refuse": the refusal lived in the Coffee flow, where the dispute state lives. The owner then fired the trigger as a pre-production gate (card t_463c1ff9).
- **Implemented (ADR-0009, card t_463c1ff9 — pre-production gate): the explicit legal-hold marker.** `config.holds` carries per-scope hold state; the hold lane (`offboarding`) **opens the hold in the same commit** (`hold_opened: true` in the ops entry) — so the dispute path cannot leave erasure unrefused by forgetting a flag; `FORGET.SCOPE { reason: 'erasure' }` on an open hold is **refused** with `legal_hold_open` (no mutation; the `operation_id` is not consumed — the same call succeeds after release); `expireRetention` **skips** held scopes (receipt `details.skipped: 'legal_hold'`, result `skipped_reason`; post-hold evidence is preserved, not tombstoned); release is the owner-only audited act **`hold.release`** (MCP `smartware_hold_release`; ops-log op `hold.release`; `operation_id` required and receipted; `no_open_hold` when nothing is open; the owner statement is recorded in the ops entry and the hold entry; a replay re-publishes a lost config release). Release does not revive the scope. Migration: a scope offboarded before the marker has no hold entry and erases as before. Pinned by `test/conformance/r_legal_hold_composition.test.ts` (R1–R6) and `q_lifecycle_composition` C8.
...
- **One scope = exactly one boundary (binding):** every content record in the package has `scope = <target>`. The only cross-scope records are (a) ops entries in the operation-closure of exported records and (b) `forget.scope` audit markers for the target — which live in the POD scope by design (they reference the scope id; content is non-PII). Manifest asserts `scope_exclusive: true` + per-source counts; the acceptance test is re-import equivalence (G4). **Implemented (t_ad51d0e2):** (b) now actually travels in `observations.jsonl` for a retained-scope package — the marker is the replay evidence that restores an offboarded scope as retained instead of resurrecting its raw window; a post-erasure package stays empty (deletion certificate only). The receiving brain rebuilds Layer 0 from the canonical log rather than incrementally catching up, so restored records below its replay watermark are not skipped.
...
| Substrate | explicit legal-hold marker | **BUILT (ADR-0009, pre-production gate, card t_463c1ff9): `config.holds` + `hold.release`** — offboarding opens the hold, erasure refused (`legal_hold_open`), sweep skips (`skipped: legal_hold`), release is an audited owner act with a receipt | tech-head | none — Coffee F3 consumes `hold.release` at the attestation step |
...
**Open @user decisions (surfaced, not decided here):** (1) Coffee release window per plan G3; (2) which flows ship in the launch cut vs later; (3) whether G3.1 export tool ships substrate-side now (recommended — cheapest, unblocks F1/F3 for any release); (4) legal-hold marker vs v1 composition — **CLOSED (2026-09-15): the marker ships (ADR-0009, pre-production gate)**; the Coffee F3 attestation step must call `hold.release` — no new open decision there.
...
- v0.15 (2026-09-14): **Lifecycle composition VERIFIED/FIXED (P1 card t_ad51d0e2 — verification pass, not a new binding).** End-to-end Coffee scenarios now run composed, on live cores: retention expiry → offboarding → erasure → export/restore → `#2` return → consolidation/correction → legal-hold conflict, each asserting the receipt, every read lane (claim, raw window, context bundle, vector) and wiped-and-rebuilt indexes (`test/conformance/q_lifecycle_composition.test.ts`, 8 tests). Three real gaps found and fixed: (1) **scope-level markers now travel with a retained-scope export** — previously an offboarded scope restored *resurrected raw evidence* in the receiving brain's raw window (export/restore equivalence break); the receiving brain now rebuilds Layer 0 from the canonical log instead of an incremental catch-up, so restored records below its replay watermark are not silently skipped, and the restore boundary admits exactly the target's scope markers; (2) **erasure attestation recorded** — `details.attestation` in the ops entry (erasure lane only, empty refused, idempotent payload identity preserved); (3) **CORRECT keeps the claim-FTS truthful** — the superseding claim was invisible to RECALL until the next rebuild (legacy handler has no index dependency; core now syncs the affected scope, mirroring CONSOLIDATE). Gate after fixes: `tsc` clean, 529/529 tests across 73 files, 31 schema files OK, `verify:saas` pass. Design positions re-verified, not changed: REVIVE after offboarding restores canonical state but stays evidence-suppressed while the scope's observations are terminal (byte-clean view rule; return path = `#2`), and the legal-hold lane is v1 composition — **an explicit legal-hold marker that makes `erasure` refuse and the sweep skip a held scope remains an open decision** (ADR-0001 AC5/AC6 vs §10c.7; no substrate hold state exists today).
- v0.16 (2026-09-14): **Legal-hold marker DECIDED — not built; v1 composition is the standing answer (P1 card t_c5c999ba → ADR-0008).** The §10c.7 open decision is closed: the substrate gains no hold state, no hold verbs and no hold config, because a hold is a legal fact the *product* knows and the substrate cannot verify — a marker would only record an intent the integrator must honor anyway, while gating a statutory DSR erasure behind a flag someone can forget to clear. Measured on a live core with an **elapsed time-bound** observation — the case C8 could not exercise, since its evidence carried the default `forever` retention, which made its "sweep purges nothing" assertion vacuous: (a) **the hold lane IS the sweep's skip** — offboarding tombstones the scope's observations in the same act, so an elapsed sweep over the held scope expires 0 and writes no bytes (skip by construction, not a flag the sweep consults); (b) **post-hold evidence is not skipped** — an elapsed sweep tombstones evidence written into the held scope after the hold, non-destructively: append-only (nothing removed), and a post-sweep F1 export of the held scope still carries every observation row with no deletion certificate, so the defense record survives every non-erasure lifecycle act; (c) **erasure is recorded, not gated** — on a held scope it succeeds with `attestation: null`, and the only substrate-side refusal on that lane is `requireOwner`. The enforceable seam is therefore the receipt (`details.attestation` + `details.export_id`) — the guarantee is "never silent", not "refuse"; the refusal lives in the Coffee flow, where the dispute state lives. ADR-0001 §4 AC5/AC6 reconciled by interpretation (realization note added; §2.2 and the criteria text are unchanged, per ADR rule 2: amending is not editing). Pinned by `test/conformance/r_legal_hold_composition.test.ts` (R1–R3). Gate: `tsc` clean, 532/532 tests across 74 files, 31 schema files OK, `verify:saas` pass. Reversal triggers (ADR-0008 §3): a real dispute/hold occurs before the client-scope flows are generally available; a second consumer needs a substrate-verifiable refusal; an erasure runs with a hold open; a tenant must prove hold state at a past instant.
- v0.17 (2026-09-15): **Legal-hold marker BUILT — the ADR-0008 trigger fired as the owner's pre-production gate (card t_463c1ff9 → ADR-0009).** The substrate now holds hold state and enforces the Tier-1 invariant literally: **`config.holds`** (per-scope; additive) carries `{opened_at/by, operation_id, released_at/by, release_operation_id, release_statement}`; **the hold lane opens it** — `FORGET.SCOPE { reason: 'offboarding' }` writes the entry in the same commit as its tombstone + grant revocation (ops entry `hold_opened: true`), so the dispute path cannot leave erasure unrefused by forgetting a flag; **erasure is refused** on an open hold (`legal_hold_open`; no mutation, no ops entry, `operation_id` unconsumed — the same call succeeds after release, idempotent replay of a committed erasure still returns its recorded result); **the sweep skips** held scopes (`details.skipped: 'legal_hold'`, result `skipped_reason`; post-hold evidence preserved, not tombstoned — this replaces the ADR-0008 §4 AC6 residual); **release is an audited owner act** — owner-only `hold.release` (MCP `smartware_hold_release`; ops op `hold.release`, added to the v0.5.0 schema enum; `payload_hash` + `statement` recorded; idempotent per `operation_id`; `no_open_hold` when nothing is open; a crash-retry finalizes from the config record). Release does not revive offboarded state. Migration: scopes offboarded before the marker have no hold entry and erase as before; payload identity for existing FORGET.SCOPE calls is unchanged (v0.5.0 hash formula pinned in R4). ADR-0008 → Superseded; ADR-0001's realization note updated (AC5/AC6 literal again). Pinned by `test/conformance/r_legal_hold_composition.test.ts` (R1–R4, rewritten to the marker) and `q_lifecycle_composition` C8. Gate: `tsc` clean, full vitest suite, `verify:schemas` 31 OK, `verify:saas` pass — recorded in `docs/journal/2026-09-15-t_463c1ff9.md`.
