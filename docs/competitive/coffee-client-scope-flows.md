...
| Legal-hold marker (ADR-0009): offboarding opens the hold in the same commit; erasure refused (`legal_hold_open`, operation_id unconsumed); sweep skips held scopes (post-hold evidence preserved); release is owner-only, receipted, idempotent per operation_id | [ADR-0009](../adr/0009-explicit-legal-hold-marker.md); `config.holds` (`src/config.ts`); `src/protocol/hold_release.ts` (`handleHoldRelease`); MCP tool `smartware_hold_release`; `test/conformance/r_legal_hold_composition.test.ts` (R1–R4) + `q_lifecycle_composition` C8 |
