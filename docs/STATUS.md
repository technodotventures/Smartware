...
**Branch:** `fix/coffee-rc-emitted-record-conformance` · **Trunk:** `main` (trunk has moves this tree does not — run `git log main..HEAD` / `git log HEAD..main`)
...
- **Journal:** [`2026-09-16-t_5ef44cc1.md`](journal/2026-09-16-t_5ef44cc1.md) — The ordinary write path's records now validate against the schemas the package publishes — and the gate reads back what it wrote (2026-09-16)
- **Journal entries:** 60 · tasks completed on board `smartware`: 86
...
| [0011](adr/0011-claim-record-materialization-block.md) | ADR-0011 — `claim.schema.json` describes the L1 record, so the extraction materialization block is enumerated in it | (no status line) |  |
...
- ✅ kanban board readable (108 tasks, 8624 events)
